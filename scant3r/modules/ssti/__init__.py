...
class Main(Scan):
    ...
    def start(self) -> Dict[str, str]:
        ...
        for method in self.opts["methods"]:
            ...
            new_urls = insert_to_params_urls(self.opts["url"], f"scan{reflect_payload}r")
            self.log.debug(f"SSTI: GENERATE A NEW URLS: {new_urls}")
                ...
                if response.__class__.__name__ == "Response":
                    ...
                    if reflect_payload in raw_response:
                        ...
                        for payload in SSTI_PAYLOADS:
                            new_custom_urls = insert_to_params_urls(self.opts["url"], payload)
                            for new_custom_url in new_custom_urls:
                                response = self.send_request(method, new_custom_url)
                                if response.__class__.__name__ == "Response":
                                    raw_response = dump_response(response)
