...
import logging
import string
from urllib.parse import urljoin, urlparse, urlencode, urlunparse, parse_qsl, urlsplit
...
def remove_dups(list_element: list) -> list:
    """
        >>> remove_dups(["item","item2","item"])
        ["item","item2"]
    """
    v = list()
    [v.append(x) for x in list_element if x not in v]
    return v


# Remove duplicate url from a list. Arg list of url. Return a clean Url list
def remove_dups_urls(list_url: list) -> list:
    """
        >>> remove_dups_urls(['http://google.com/?test=1',
                            'http://php.net/?test=1',
                            'http://google.com/?test=1'])
        ['http://google.com/?test=1','http://php.net/?test=1']
    """
    v = list()
    [v.append(i) for i in list_url if i not in v and urlparse(i).netloc]
    return v
...
def insert_to_custom_params(url: str,
                            parameter: str,
                            text: str,
                            remove_content: bool = False) -> str:
    """
...
def insert_to_params_urls(url: str,
                          text: str,
                          remove_content: bool = False) -> list:
    """
...
def insert_text_to_urlpath(url: str, text: str) -> list:
    ...
    try:
        ...
        for path_element in path.split('/'):
            if path_element:
                path_index = url.index(path_element)
                u = list(url)
                u[path_index] = path_element + text
...
def insert_after(text: str, find_this: str, new_text: str) -> str:
    """
