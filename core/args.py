...
class Args:
    ...
    def set_argument(self, options: list, name: str, parser: argparse.ArgumentParser):
        ...
        if dict_option["type"] != bool:
            parser.add_argument(
                dict_option["small"],
                dict_option["large"],
                dest=dict_option["name"],
                type=dict_option["type"],
                default=dict_option["default"],
                action=dict_option["action"],
    ...
    def create_dict_args(self, current_args: argparse.Namespace) -> Dict[Any, Any]:
        dict_args = {"urls": []}
        ...
        for name, value in args.items():
            if value == 0 or value == [] or value == {} or value == False:
                ...
                dict_exe = list(
                    filter(lambda item: "exec" in item.keys(), self.conf[name])
                )[0]
                if dict_exe:
                    exec(dict_exe["exec"])
        return dict_args
    ...
    def get_args(self) -> dict:
        parser = argparse.ArgumentParser(
            epilog=self.epilog_text(), formatter_class=argparse.RawTextHelpFormatter
        )
